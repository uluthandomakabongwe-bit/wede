# SWEETHEART FACTORIE
ST10240259
Uluthando Shongwe
WEDE520
 
This project focuses on designing and developing a website for Sweetheart Factories, a business that specializes in thoughtful and affordable gift sets for newborns and toddlers.

The website aims to improve the company’s online presence, increase customer engagement, and provide customers with a convenient way to browse and purchase products online.
 
 
 # Sweetheart Factorie orgaanisation overview
 On the 30th January 2026, an interview was conducted with the owners of Sweetheart Factories to gather information about the business operations, website requirements, and objectives. This interview provided valuable insights into the business and its goals.
 


# Project Overviews
 
This repository contains the front-end development for the Sweetheart Factorie website, transitioning the brand from social-media-only sales to a dedicated online presence.
• Mission: To provide high-quality, budget-friendly gifts for new mothers with a focus on convenience and care.
• Vision: To be the first choice for thoughtful and accessible gifts for mommies.

 # Key Features & Functionality
• Custom Gift Packs: An interactive catalog allowing customers to build their own gift packs.
• Direct Ordering: Integrated "Add to Cart" and checkout functionality.
• Responsive Design: Fully mobile-friendly UI optimized for busy parents on the go.
• Catalog System: Organized product categories for nursery items, toys, and strollers.

# Target audience
Moms
Dads
Family members
Friends looking for baby gifts
Parents of newborns and toddlers

# Website Goals & Objectives
The main objectives of the website are to:

-Increase brand awareness
-Improve online visibility
-Build customer loyalty
-Promote specials and discounts
-Accept online orders
-Allow customers to build custom gift packs
-Sell products online efficiently

# Key Performance Indicators (KPIs):
The success of the website will be measured using:
-Number of online orders placed
-Monthly website traffic
-Number of new customers
-Click-through rate (CTR)
-Customer satisfaction score

 # Features:
-Online shopping functionality
-Custom gift pack builder
-Responsive design
- Product browsing and search
- Product catalog
-Contact page

# Design & User Experience
The website follows a "kid-focused yet professional" aesthetic to ensure trust and ease of use.
• Color Palette:
• Background: Beige (#F2EAE0) — Harmonious and neutral.
• Primary: Orchid Pink (#E4A3D4) — Energetic, playful, and friendly.
• Text: Charcoal Black (#263238) — High contrast for optimal readability.
• Typography:
• Headings: Quicksand or Fredoka for a friendly, rounded feel.
• Body: Open Sans, Lato, or Nunito for clean readability.
• Layout: A neat grid layout for product browsing with a "sticky" navigation bar for quick access to the cart and search.

# technincal requirments: 
• HTML5: Semantic structure for SEO and accessibility.
• CSS3: Custom styling featuring Flexbox/Grid for the responsive catalog.
• Images: Sourced from FreePik to maintain high-quality visual standards.# wede

# Timeline 
6 April - start of project and research 
9-15 - content planning
15-26 - HTML foundation
28 - sumbit project


# SITEMAP /project structure

├── index.html          # Homepage with hero slider and featured promos
├── about.html          # Company history and mission statement
├── catalog.html        # Product display with package options
├── order.html          # Online order page and shopping cart
└── assets/             # Images, CSS, and brand assets

#common website structure:
<header>
   
    <!-- Navigation Menu -->
</header>

<main>

    <!-- Hero / Banner Section -->

    <!-- Main Page Content -->

    <!-- Additional Sections -->

</main>

<footer>
  
    <!-- Copyright -->
</footer>

# changelog:
- Initial project setup
- Home page structure
- About page structure
- Catalogue page structure
- Order page structure
- Contact page structure
-Common website layout
- Navigation bar
- Footer section
- Responsive design planning
- README documentation
-GitHub project setup
##Part 2 (25-29 May 2026)

### About Page (`about.html`)

* Added responsive owner profile section using Flexbox.
* Created side-by-side image and text layout with `.person-card`.
* Added fixed image sizing and fallback styling for missing images.
* Implemented mobile-responsive breakpoints for tablets and phones.
* Added image hover effects and fade-in animations.
* Simplified HTML structure using `.person-block` containers.

### Catalogue Page (`catalogue.html`)

* Fixed text and HTML entity issues.
* Completed unfinished product information.
* Improved product descriptions and grammar.
* Corrected typos and formatting inconsistencies.
* Removed duplicate closing tags.
* Updated footer text and styling for consistency.

### Stylesheet Updates (`styles.css`)

* Added `.owner-section` styling.
* Improved image presentation and responsiveness.
* Enhanced overall layout consistency across devices.


# Design
-Added wireframe-based page layouts
-Planned mobile-friendly interface

# Technical
-Created HTML page skeletons

# Reference list: 
 
Afrihost,2026. Domains[Online]. Avaliable at:https://www.afrihost.com/domains.[Accessed on 12 April 2026].

Sweetheart Factorie,2026.Interview with Owners Regarding Business Operations and Website Needs. Conducted by UM shongwe,07April 2026

ColorHunt,n.d. Color Palettes for Designers and Artists.[Online]. Avaliable at: https://colorhunt.co.[Accessed on 12 April 2026] 

FreePik,n.d.images.[Online].Available at: https://www.freepik.com. [Accessed on 12  April 2026]
